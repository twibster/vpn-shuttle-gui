...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        config_label = Gtk.Label(label="VPN:")
        config_box.append(config_label)

        self._config_dropdown = Gtk.DropDown()
        self._config_dropdown.set_size_request(200, -1)
        config_box.append(self._config_dropdown)

        refresh_btn = Gtk.Button()
        refresh_btn.set_icon_name("view-refresh-symbolic")
        refresh_btn.set_tooltip_text("Refresh config list")
        refresh_btn.connect("clicked", lambda b: self._refresh_configs())
        config_box.append(refresh_btn)

        header.pack_start(config_box)
    ...
    def _refresh_configs(self):
        def load():
            ...
            GLib.idle_add(self._populate_dropdown, configs)

        threading.Thread(target=load, daemon=True).start()

    def _populate_dropdown(self, configs):
    ...
    def _on_settings_clicked(self, button):
        dialog = SettingsDialog(
            self, self._config, self._backend, on_config_changed=self._refresh_configs
        )
        dialog.present()
